...
def coltrane_cost_function(params, trait, forcing, obs):
    """
    ...
    params: dict
        Parameters to test.
    trait: string
        Name of the trait on which to compute the RMSE.
        Ex: "R" for lipid reserves and 'R_and_fullness' for both traits
    forcing: dict
    ...
    if trait == "R":
        ...
        if 2 in unique: # The run gave adults in august that have reproduced
            ...
            ### Compute the mean reserves for those individuals for each strategy
            ...
            # Raw distributions
            cost_lip, obs_interp_lip, mod_interp_lip, bins_lip = cost_function(obs, adults_august_repro_mean_reserves_tranf_all)
            ...
            cost_lip_wgt, obs_interp_lip, mod_interp_lip_wgt, bins_lip_wgt = cost_function(obs, adults_august_repro_mean_reserves_tranf_wgt_all)
            
            # Median reserves
            med_obs = np.nanmedian(obs)
            med_reserves = np.nanmedian(adults_august_repro_mean_reserves_tranf_all)
            
            cost_lip_med = np.mean((med_reserves - med_obs) ** 2)
            
            # Total 
            cost_tot = cost_lip + cost_lip_wgt + (cost_lip_med/10)
